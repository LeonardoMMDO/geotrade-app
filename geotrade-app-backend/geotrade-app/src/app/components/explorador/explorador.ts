...
export class ExploradorComponent implements OnInit, AfterViewInit, OnDestroy {
  private readonly bizMarkerByCategory: { [key: string]: { color: string } } = {
  restaurante:    { color: '#e67e22' },
  abarrotes:      { color: '#27ae60' },
  estetica:       { color: '#e84393' },
  ferreteria:     { color: '#7f8c8d' },
  panaderia:      { color: '#d35400' },
  papeleria:      { color: '#2980b9' },
  farmacia:       { color: '#c0392b' },
  farmacias:      { color: '#c0392b' },
  gym:            { color: '#1f7a8c' },
  floreria:       { color: '#d63384' },
  boutique:       { color: '#6f42c1' },
  veterinaria:    { color: '#16a085' },
  veterinarias:   { color: '#16a085' },
  refaccionaria:  { color: '#34495e' },
  refaccionarias: { color: '#34495e' }
};
...
cerrarSesion() {
          ...
          if (biz.latitud != null && biz.longitud != null) {
            ...
            if (bizCategory.includes(tipoNormalizado) || tipoNormalizado.includes(bizCategory) || bizCategory === tipoNormalizado) {
              ...
              const marker = new google.maps.Marker({
                ...
                icon: this.getBizMarkerIcon(biz.categoria_empresa, this.getEmojiParaCategoria(biz.categoria_empresa)),
  ...
  private getEmojiParaCategoria(categoriaNombre: string | null | undefined): string | undefined {
  if (!this.categories?.length) return undefined;
  const normalized = this.normalizeCategoryName(categoriaNombre);
  const match = this.categories.find(c =>
    this.normalizeCategoryName(c.label) === normalized ||
    this.normalizeCategoryName(c.value) === normalized
  );
  return match?.mapIcon || undefined;
}
  ...
  private normalizeCategoryName(value: string | null | undefined): string {
    ...
    private getBizMarkerIcon(categoria: string | null | undefined, emojiMapa?: string): google.maps.Icon {
      ...
      const fallbackStyle = this.bizMarkerByCategory[matched || ''] || { color: '#e74c3c' };
      const markerStyle = {
        glyph: emojiMapa || '🏪',
